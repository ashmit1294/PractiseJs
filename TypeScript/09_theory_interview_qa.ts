...
/*
Q11 [ADVANCED]: Full-stack TypeScript — building type-safe applications end-to-end
──────────────────────────────────────────────────────────────────────────────────
A: End-to-end type safety from database to UI requires each layer to derive types
   from the layer below (single source of truth). No manual type duplication.\n   Strategy: Database → ORM types → API types → Client types → React components.\n   Tools: Prisma (ORM → types), tRPC (TypeScript-native RPC), GraphQL Codegen.
*/\n\n// 1. DATABASE SCHEMA (Prisma):\n// prisma/schema.prisma:\n// model User {\n//   id    Int     @id @default(autoincrement())\n//   email String  @unique\n//   name  String?\n//   posts Post[]\n// }\n// model Post {\n//   id       Int     @id @default(autoincrement())\n//   title    String\n//   author   User    @relation(fields: [authorId], references: [id])\n//   authorId Int\n// }\n\n// Prisma generates types automatically:\n// import { User, Post } from '@prisma/client';\n\n// 2. BACKEND (Node.js + Express + Prisma):\ninterface CreateUserInput {\n  email: string;\n  name: string;\n}\n\ninterface CreateUserOutput {\n  id: number;\n  email: string;\n  name: string | null;\n}\n\n// API endpoint:\n// app.post('/api/users', async (req, res) => {\n//   const input: CreateUserInput = req.body;\n//   const user = await prisma.user.create({ data: input });\n//   const output: CreateUserOutput = user;\n//   res.json(output);\n// });\n\n// 3. API CLIENT (React + Fetch):\nclass UserAPI {\n  async createUser(data: CreateUserInput): Promise<CreateUserOutput> {\n    const res = await fetch('/api/users', {\n      method: 'POST',\n      body: JSON.stringify(data),\n      headers: { 'Content-Type': 'application/json' },\n    });\n    return res.json() as Promise<CreateUserOutput>;\n  }\n}\n\n// 4. REACT COMPONENT (consumes API types):\nfunction CreateUserForm() {\n  const [formData, setFormData] = useState<CreateUserInput>({\n    email: '',\n    name: '',\n  });\n\n  const handleSubmit = async (e: React.FormEvent) => {\n    e.preventDefault();\n    const user: CreateUserOutput = await new UserAPI().createUser(formData);\n    console.log('Created user ID:', user.id);  // ← type-safe\n  };\n\n  return (\n    <form onSubmit={handleSubmit}>\n      <input value={formData.email} placeholder=\"Email\" />\n      <button>Create</button>\n    </form>\n  );\n}\n\n// BEST PRACTICE: Use tRPC for maximum type safety (end-to-end without codegen):\n// tRPC is TypeScript-native — backend routes are typed as functions in frontend.\n// No code generation, no duplication — one source of truth (backend router type).\n\n// backend/router.ts:\n// import { z } from 'zod';\n// import { publicProcedure, router } from './trpc';\n// export const appRouter = router({\n//   user: router({\n//     create: publicProcedure\n//       .input(z.object({ email: z.string().email(), name: z.string().optional() }))\n//       .mutation(async ({ input }) => {\n//         return await prisma.user.create({ data: input });\n//       }),\n//   }),\n// });\n// export type AppRouter = typeof appRouter;\n\n// frontend/page.tsx — FULLY TYPED with zero codegen:\n// import { trpc } from '@/lib/trpc';\n// function CreateUserForm() {\n//   const createMutation = trpc.user.create.useMutation();\n//   const handleCreate = () => {\n//     createMutation.mutate({ email: 'test@example.com', name: 'Test' });\n//     // TypeScript checks:\n//     // - input types match backend .input()\n//     // - result type is inferred from backend resolver return\n//   };\n// }\n\nexport { UserRepository };\nexport type { Repository, UserId, ProductId, ValidEmail, RouteParams, DeepReadonly };
