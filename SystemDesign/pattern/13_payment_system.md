...
|---|---|---|
| **Stripe** | Stripe Payments | The gold standard payment API. Stripe's architecture is built on every pattern described: idempotency keys on every API call (you can retry a charge request with the same idempotency key and it will never double-charge), webhook delivery with automatic retry, event sourcing for payment state (every state transition is an event), SAGA for complex flows (SetupIntent → PaymentIntent → Charge). Stripe's "Radar" fraud system uses ML to score every payment in real-time. Stripe processes hundreds of billions of dollars per year. |
| **PayPal** | PayPal / Venmo | One of the original internet payment systems (1998). PayPal pioneered many patterns now standard: sender pays (vs card-present merchant pays) fee model, async settlement (money "pending" in PayPal balance before bank settlement), dispute resolution workflow (the SAGA pattern where PayPal can claw back funds days after initial payment — compensating transactions). PayPal's architecture evolved from a monolith to microservices, documented in their engineering blog as "PayPal's Kraken" service framework. |
| **Razorpay** | Razorpay (India) | India's leading payment gateway. Built to handle UPI (Unified Payments Interface) — India's real-time bank transfer protocol that settles in 10 seconds flat (vs card settlement taking T+2 days). Razorpay's architecture must handle: (1) UPI payment requests that time out at the bank level needing status polling, (2) reconciliation across 50+ Indian banks with inconsistent APIs, (3) compliance with RBI (Reserve Bank of India) regulations requiring data localization (payment data must stay in India). |
| **Shopify Payments** | Shopify Payments | Embedded payment processor for Shopify merchants. Key design: payment processing is tightly integrated with order management (same platform). When a payment is captured, inventory is automatically decremented and fulfillment flows triggered — the SAGA pattern with automatic compensations. Shopify's challenge: millions of independent merchants, each with their own fraud risk profile and currency — multi-tenant payment system with per-merchant risk settings. |
| **Wise (formerly TransferWise)** | International Money Transfer | The payment system for FX transfers. Wise's innovation: instead of actually sending money internationally, they maintain local bank accounts in each country and net internal transfers. User sends GBP to Wise UK → Wise UK pays GBP to another user's GBP account → separately, Wise US pays USD from their US account to recipient. No international wire transfer happens — it's accounting ledger entries. The reconciliation system is the critical piece: matching that GBP sent and USD received balance across all corridors. |
| **Adyen** | Adyen Global Payments | Payment infrastructure used by Netflix, Spotify, Uber, LinkedIn. Adyen operates their own acquiring bank licenses rather than routing through intermediaries. Their "unified commerce" platform processes in-store terminal payments (POS) and online payments through the same API and ledger — a single reconciliation view across all channels, all geographies. Single-ledger multi-currency reconciliation at enterprise scale is Adyen's flagship differentiator. |
