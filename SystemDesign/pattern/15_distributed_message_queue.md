...
|---|---|---|
| **LinkedIn** | Apache Kafka (LinkedIn origin) | Kafka was invented at LinkedIn in 2011 to solve their activity data pipeline problem. LinkedIn's activity stream (page views, clicks, connections, profile views) was too high-volume for traditional queues. Kafka was purpose-built for log-based, append-only, high-throughput event streaming. LinkedIn still runs one of the largest Kafka deployments: 7 trillion messages/day, 100+ Kafka clusters, 700+ billion messages/day at peak. They open-sourced Kafka in 2011; it became the industry-standard event streaming platform. |
| **Uber** | Kafka for Trip Events | Uber's "uReL" real-time location service publishes 1 million GPS updates per second to Kafka topics. Consumers include: driver matching (reads location to find nearby drivers), surge pricing (reads trip request events to calculate supply/demand ratio), dispatch (reads trip events), analytics (reads all events for business intelligence). Multiple independent consumer groups each processing the same Kafka stream — the exact pattern described. Uber's engineering posts detail their Kafka cluster management at scale. |
| **Netflix** | Keystone Pipeline | Netflix's data pipeline (named "Keystone") is built on Kafka. Every play event, pause, buffering event, subtitle change, and skip generates a Kafka message. Downstream consumers: recommendation ML pipeline (train models on viewing behavior), A/B test analysis (measure which UI variant leads to more play events), content analytics (which shows are watched and where). Netflix published that Kafka handles 700 billion events/day across all their pipeline topics. |
| **AWS** | Amazon SQS / SNS | AWS SQS is the managed message queue (traditional queue — message deleted after delivery, not log-based). SNS is the fan-out layer (publish once, deliver to multiple SQS queues, Lambda functions, email endpoints). The SNS + SQS pattern is the AWS-managed equivalent of Kafka topics + consumer groups. SQS FIFO queues ensure ordering within a message group. Used by millions of AWS workloads as the default "decouple services" pattern. DLQ support is native to SQS. |
| **Cloudflare** | Cloudflare Queues | Cloudflare's managed message queue (launched 2023) built specifically for Cloudflare Workers (edge computing). Demonstrates the emerging pattern: message queues as first-class primitives in edge computing platforms. At-least-once delivery, explicit message acknowledgement, dead letter queue — all the same patterns but optimized for sub-millisecond edge latency rather than data center throughput. |
| **Shopify** | Kafka for Commerce Events | Shopify runs Kafka to decouple their commerce platform: a new order event is published once and consumed by: inventory service (decrease stock), fulfillment service (trigger shipping), notification service (email customer), analytics (revenue metrics), fraud detection (score the order). Six independent consumers, one event. This decoupling is what allows Shopify to handle Black Friday peaks (tens of thousands of orders/second) without any single system becoming a bottleneck. |
