...
|---|---|---|
| **Google** | Googlebot | The most sophisticated crawler ever built. Crawls billions of pages daily. Key proprietary elements: distributed across data centers worldwide (each region crawls geographically closer sites). Politeness is respected (robots.txt, Crawl-delay) — Google explicitly states they throttle when servers are slow. JavaScript rendering queue ("Secondary wave indexing") processes SPAs separately with a delay. Google's index is built from crawler output via an inverted index pipeline (Bigtable + Colossus storage). |
| **Bing** | Bingbot | Microsoft's crawler. Similar architecture to Googlebot. Bingbot publishes detailed crawl statistics in Bing Webmaster Tools, showing crawls/day, discovery rate, and index freshness per domain. Bing's differential from Google: Bing partners with LinkedIn (same Microsoft company) and factors in LinkedIn professional signals for some result rankings. |
| **Common Crawl** | CC-MAIN corpus | A non-profit that runs a monthly open web crawl and publishes the raw data for AI training and research. The Common Crawl corpus (100TB+ per crawl) is stored on AWS S3 (public dataset). Used to train GPT-3, LLaMA, and virtually every large language model. Their crawler architecture is open-source: Apache Nutch-based, runs on AWS EMR (Hadoop). The entire pattern — frontier queue, politeness, deduplication, S3 storage — is documented publicly. |
| **Ahrefs / SEMrush** | SEO Crawler | Commercial SEO platforms run their own crawlers (Ahrefs Bot, SemrushBot) to build backlink indices. Ahrefs claims to crawl 8 billion pages/day — making it the second most active crawler after Googlebot. The output is their proprietary backlink database (who links to whom) — an inverted index of links rather than content. |
| **Scrapy Cloud / Apify** | Hosted Web Scraping | The open-source framework (Scrapy) and its SaaS wrapper (Scrapy Cloud, Apify) implement exactly the architecture described — URL frontier, polite scheduler, content extractor, output to S3/database. Used by thousands of companies to build price monitors, competitor trackers, and data pipelines. Apify marketplace shows 5,000+ pre-built crawlers for specific websites. |
| **Internet Archive** | Wayback Machine | Crawls and archives the web to preserve history. Stores petabytes of historical snapshots. The Wayback Machine is a web crawler + time-series content store: for each URL, it keeps every snapshot taken over the years. Demonstrates the long-retention, append-only storage variant of the crawler output model. |
