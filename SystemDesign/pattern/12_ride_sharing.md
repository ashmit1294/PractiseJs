...
|---|---|---|
| **Uber** | Uber (original creator of this architecture) | Uber's engineering blog is the primary source for this entire design. Key published details: H3 geospatial indexing (Uber's open-source hexagonal grid replacing geohash) for driver lookup. "Ringpop" for consistent hashing across dispatch servers. "Schemaless" (Uber's custom Cassandra-based storage) for trip data. "uReL" for real-time location streaming (Kafka at 1M location updates/second). Surge pricing ML model evolved from a lookup table to a deep learning model. Uber published "Project Rider" to explain geospatial matching to the public. |
| **Lyft** | Lyft | Uber's primary US competitor. Very similar architecture (both use Kafka, Redis Geo, microservices). Lyft's key differentiation: "Prime Time" pricing (surge equivalent) displayed as a percentage addition rather than a multiplier — human psychology difference. Engineering blog: Lyft uses "Repartitioner" for Kafka consumer group management. Lyft's matching system "Dispatch" is documented as assigning driver-to-rider pairs using a Hungarian algorithm variant for optimal global matching (not just nearest driver). |
| **DiDi** | DiDi (China's dominant ride-share) | Largest ride-sharing company by trips (more than Uber in China + international). DiDi's unique challenge: traffic in Chinese megacities (Beijing, Shanghai) is among the most complex in the world. DiDi invested heavily in ML-based routing with real-time traffic prediction. Published research on "order dispatching at DiDi via reinforcement learning" — their dispatch system uses RL to match supply and demand at a fleet level rather than greedy local matching. |
| **Ola** | Ola (India) | India's dominant ride-share. Ola's engineering challenge: extremely diverse network conditions (2G to 5G within the same city), high driver phone turnover (cheap Android devices), and payment infrastructure dominated by UPI/cash rather than cards. Architecture adaptations: progressive web app for low-end phones, offline-capable driver app that queues GPS updates during connectivity loss, UPI payment integration for instant settlement. |
| **DoorDash** | DoorDash Delivery Matching | Delivery logistics variant: same spatial matching (dasher location in Redis Geo) but with three-way matching (consumer order → restaurant ready time → dasher proximity). DoorDash's "Batching" algorithm assigns one dasher to pick up multiple orders from nearby restaurants — requires solving a travelling salesman variant in real-time. Published engineering post on Iguazu (their real-time dispatch service). |
| **Google Maps** | Live Traffic + ETA | The infrastructure layer that ride-sharing apps use for ETA. Google Maps processes billions of location pings daily from Android phones (in aggregate, anonymized) to create real-time traffic layers. The location data pipeline is architecturally identical: GPS points → Kafka → stream processing → speed-per-road-segment updates → routing graph. Makes Google Maps a real-world example of the same location streaming architecture at 10× the scale of any ride-sharing app. |
