...
|---|---|---|
| **WhatsApp** | WhatsApp Messenger (2B users) | The engineering benchmark for chat architecture. Erlang-based servers (Ejabberd, heavily modified) handle persistent WebSocket/XMPP connections. Message storage: messages are stored on WhatsApp servers only until delivered, then deleted (not stored long-term — privacy by design). E2E encryption using the Signal Protocol means WhatsApp servers route encrypted blobs without being able to read content. Group chats with 1024-member limit use fan-out on write. |
| **Slack** | Slack (10M+ daily active users) | Enterprise-grade chat. Messages stored permanently in Cassandra-like storage (Slack uses MySQL and Memcached historically, evolved to a more complex stack). Key feature: channels replace the concept of DMs-as-primary-communication. Slack's "Flannel" (their WebSocket edge layer) manages persistent connections. Slack's real-time presence (green dot) uses the same Redis Pub/Sub pattern for presence events. |
| **Telegram** | Telegram | Focus on large groups (up to 200,000 members) and channels (broadcast, unlimited subscribers). Uses MTProto protocol (custom binary protocol more efficient than XMPP/WebSocket for mobile). Messages stored on Telegram's servers (cloud-synced across devices). Fan-out on read for channels: a message to a 10M-subscriber channel is stored once; subscribers fetch it on demand. |
| **Discord** | Discord | Chat + voice + video for gaming communities. Known for their engineering blog posts on scaling. Architecture highlight: their "Elixir" (Erlang VM) Chat Gateway handles millions of persistent WebSocket connections. "Guilds" (servers) with 250,000 members required moving from fan-out on write to event broadcasting. In 2023, Discord moved message storage from Cassandra to ScyllaDB for better performance at their scale. |
| **iMessage / Apple** | iMessage | Uses APNs (Apple Push Notification Service) as the delivery layer for all messages. When both sender and receiver are online, P2P delivery path. When offline, APNs queues. E2E encrypted. The "blue bubble vs green bubble" distinction is iMessage (E2E encrypted, stored on Apple's servers briefly) vs SMS (not encrypted, routed through carrier). |
| **Microsoft** | Microsoft Teams | Built on Azure Communication Services. WebSocket connections through Azure's global network. Enterprise features like compliance recording and legal hold mean messages are stored long-term (unlike WhatsApp's delete-on-delivery). Teams uses a modified fan-out for @channel mentions (notify all) vs regular messages (notify active participants only). |
