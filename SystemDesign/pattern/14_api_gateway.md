...
|---|---|---|
| **Netflix** | Netflix Zuul (open-sourced) | Netflix built Zuul as their API Gateway and open-sourced it. Zuul handles all inbound traffic to Netflix's 700+ microservices. Key features: dynamic routing filters written in Groovy (hot-deployed without gateway restart), request/response transformation, sticky sessions for A/B testing (same user always routes to the same backend version), and custom resilience via Ribbon (client-side LB) + Hystrix (circuit breaker). Zuul 2 rewrote on Netty for async I/O. This is the real-world implementation of the exact pattern described. |
| **AWS** | AWS API Gateway | The managed API Gateway service. Behind every serverless API built with Lambda + API Gateway is this pattern: request arrives → API GW validates API key → authorizer Lambda checks JWT → routes to Lambda or HTTP backend. Supports rate limiting per API key, usage plans (monthly quotas), request/response transformation, caching (built-in 5-minute TTL cache for GET responses). AWS API Gateway powers millions of production APIs. |
| **Kong** | Kong Gateway (open-source) | The most popular open-source API Gateway (used by T-Mobile, Nasdaq, Honeywell). Kong is built on top of Nginx + OpenResty (Lua-based). Plugins (auth, rate limiting, logging, transformation) are exactly the cross-cutting concern pattern. Kong's declarative config (`kong.yml`) lets you describe all routes, services, and plugins in code — GitOps for API Gateway. Kong Konnect is the managed SaaS version. |
| **Cloudflare** | Cloudflare Workers + API Shield | Cloudflare operates at the edge (300+ data centers worldwide) and is the outermost API Gateway layer for many companies. API Shield provides schema validation (reject requests that don't match OpenAPI spec before they reach origin), mTLS client certificates, and bot detection — all cross-cutting concerns executed at the CDN edge, not in your application. Cloudflare Workers allows custom gateway logic (JWT validation, request routing) to run in V8 isolates at the edge with ~1ms cold start. |
| **Apigee (Google)** | Apigee API Management | Enterprise API Gateway used by telcos, banks, and large enterprises. Apigee's differentiator: full API lifecycle management — gateway + developer portal + analytics + monetization (charge per API call) in one product. A telecom company exposing their network APIs (SMS, voice) to third-party developers uses Apigee to handle auth, rate limiting, and metered billing in one platform. |
| **Envoy Proxy / Istio** | Service Mesh + Ingress | Envoy is the data plane behind many gateways (AWS App Mesh, Istio, Contour). As an ingress gateway (north-south), Envoy handles all the patterns described. As a sidecar in a service mesh (east-west via Istio), every microservice gets its own Envoy sidecar that applies rate limiting, circuit breaking, and mTLS between services — the cross-cutting concerns move from a central gateway to a distributed mesh. Both patterns coexist in large organizations. |
