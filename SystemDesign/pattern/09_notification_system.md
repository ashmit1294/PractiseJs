...
|---|---|---|
| **Facebook** | Facebook Notifications | Pioneered the multi-channel notification system. When someone likes your post, the event flows through Kafka, fan-out service evaluates your notification preferences, deduplication prevents 10 likes in 10 seconds from sending 10 separate notifications (batching into "10 people liked your post"). Delivery via in-app notification badge + push + optional email. The batching/aggregation logic is the differentiating feature vs simpler systems. |
| **Uber** | Uber Rider/Driver Alerts | Real-time high-priority notifications that must arrive within 2 seconds. "Your driver is arriving in 1 minute" — late delivery makes the notification useless. Uber uses APNs/FCM direct path with minimal queueing for these time-critical alerts. A separate lower-priority queue handles promotional notifications ("20% off your next ride") where a 5-minute delay is acceptable. Clear priority-tier separation in the queue architecture. |
| **LinkedIn** | LinkedIn Notifications | Famous for heavy A/B testing of notification copy. "John viewed your profile" is one of LinkedIn's highest-engagement notification types (curiosity-driven). LinkedIn tests notification timing (morning vs evening by timezone), copy variants, and frequency caps using their notification experimentation platform. They also pioneered aggressive unsubscribe compliance — "Email settings" page with 30+ granular controls. |
| **Twilio** | Twilio Notify / SendGrid | The infrastructure behind millions of notification systems. Twilio provides the final-mile delivery: SMS (Twilio Messaging), email (SendGrid), WhatsApp Business API, and voice. Many companies build their notification service on top of Twilio as the last-mile provider, handling their own preference management and queueing internally. Twilio's webhook system reports delivery status back — exactly the feedback loop described in the architecture. |
| **Airbnb** | Booking Notification | Complex multi-channel orchestration: a booking event triggers email + SMS + app push simultaneously, but with coordination: if the user opens the app within 30 seconds (implicit delivery confirmed), suppress the SMS and email to avoid notification fatigue. Airbnb's "smart suppression" logic is a key feature of their notification system — cross-channel awareness rather than treating each channel independently. |
| **Duolingo** | Streak Reminder Push | Famous for aggressive (sometimes mocked) push notification strategy. "You’re about to lose your streak!" notifications are sent at personalized times based on when each user historically opens the app. ML-driven timing optimization: the notification system learns the optimal send time per user to maximize open rate. High-frequency notification system with per-user cadence learning — the recommendation ML applied to notification timing. |
