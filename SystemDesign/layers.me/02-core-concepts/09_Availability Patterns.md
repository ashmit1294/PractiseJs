...
- You pick the pattern based on **uptime SLA (Service Level Agreement) + cost of downtime vs cost of redundancy**
...
Battery backup   = Cache/CDN (Content Delivery Network) layer
...
- **RTO (Recovery Time Objective)** (Recovery Time Objective) = time between failure and restored service
- **RPO (Recovery Point Objective)** (Recovery Point Objective) = amount of data you can lose during a failure
- Sync replication → RPO (Recovery Point Objective) ≈ 0 (zero data loss), but adds latency
- Async replication → RPO (Recovery Point Objective) > 0 (may lose recent writes), but faster
...
ACTIVE-PASSIVE:
  ...
  RTO (Recovery Time Objective): 30s–5min   Cost: 50% utilization   Consistency: Simple ✓

ACTIVE-ACTIVE:
  Load Balancer → Server 1 (50% traffic)
              → Server 2 (50% traffic)
  RTO (Recovery Time Objective): ~0s    Cost: 100% utilization   Consistency: Complex ⚠️
```
...
|---|---|---|
| **RTO (Recovery Time Objective)** | 30s – 5min | Near-zero |
...
| **When to use** | Moderate SLA (Service Level Agreement), financial/strong-consistency workloads | High SLA (Service Level Agreement), high-volume, eventual consistency acceptable |
...
If one region fails → DNS (Domain Name System) routes to next nearest → ~instant recovery
```
...
- **BUT** payments use active-passive: sync replication to standby, 30s RTO (Recovery Time Objective)
...
### Netflix — Streaming
- **Active-Active across 3 AWS (Amazon Web Services) regions** (US-East, US-West, EU)
- Automated failover; analytics only needs 99% availability
- Streaming needs 99.99% → active-active is justified
- Discovered DNS (Domain Name System) was a single point of failure → now uses **multiple DNS (Domain Name System) providers**
...
| **Over-engineering** | "Active-active multi-region" sounds impressive but may cost millions for a 99.9% SLA (Service Level Agreement) problem |
| **Forgetting the LB itself** | Load balancer = single point of failure too. Make it redundant (VRRP, AWS (Amazon Web Services) ELB). |
| **Theoretical RTO (Recovery Time Objective) ≠ actual RTO (Recovery Time Objective)** | Always test failover with chaos engineering; measure real recovery time |
...
| Level | Expectation |
|---|---|
| **Mid** | Explain active-passive vs active-active with examples; identify SPOFs; know RTO (Recovery Time Objective) concept; know failover needs health checks |
| **Senior** | Pick pattern based on SLA (Service Level Agreement) + consistency needs; quantify cost vs downtime; discuss correlated failures; mention RTO (Recovery Time Objective)/RPO (Recovery Point Objective) tradeoffs; hybrid designs |
...
**Common Questions:**
- "Design a highly available system for X" → Ask SLA (Service Level Agreement) first → choose pattern → justify
- "Active-passive vs active-active — when to use each?" → RTO (Recovery Time Objective): near-zero vs 30–300s; cost vs consistency
...
- Not asking about SLA (Service Level Agreement) before choosing a pattern
- Forgetting the load balancer is also a SPOF (Single Point of Failure)
...
2. **Active-Passive**: 1 primary + 1 standby → simple, cheap, 30s–5min RTO (Recovery Time Objective)
3. **Active-Active**: all nodes serve traffic → near-zero RTO (Recovery Time Objective), but complex + expensive
4. **Match pattern to SLA (Service Level Agreement)**: 99.9% → active-passive; 99.99%+ → active-active
5. **Consistency constraint**: financial/inventory → active-passive; social/analytics → active-active
6. **Hybrid is common**: active-active for stateless API (Application Programming Interface)/dispatch + active-passive for payments/DB
7. **Test failover regularly** — theoretical RTO (Recovery Time Objective) means nothing without validation
...
`availability patterns` `active-passive` `active-active` `hot standby` `warm standby` `cold standby` `failover` `replication` `single point of failure (SPOF)` `RTO (Recovery Time Objective)` `RPO (Recovery Point Objective)` `health checks` `load balancer redundancy` `multi-AZ` `multi-region` `geographic redundancy` `correlated failures` `chaos engineering` `99.99% availability` `SLA (Service Level Agreement)` `master-slave` `multi-master` `quorum writes`
